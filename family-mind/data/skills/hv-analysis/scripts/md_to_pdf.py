...
def main():
    ...
    # 转 PDF（优先 WeasyPrint，失败时回退到 Chrome headless）
    pdf_generated = False
    try:
        from weasyprint import HTML as WPHTML
        WPHTML(string=html).write_pdf(args.output)
        pdf_generated = True
        size_kb = os.path.getsize(args.output) / 1024
        print(f"[OK] PDF 已生成（WeasyPrint）: {args.output} ({size_kb:.1f} KB)")
    except Exception as e:
        print(f"[WARN] WeasyPrint 失败: {e}")
        print("[INFO] 尝试使用 Chrome headless 回退...")

    if not pdf_generated:
        # Chrome headless 回退方案
        import subprocess
        chrome_paths = [
        ...
        if chrome_bin is None:
            print("[ERROR] 未找到 Chrome/Chromium，PDF 生成失败。")
            print("[HINT] macOS 请安装 Google Chrome；Linux 请安装 chromium 或 google-chrome-stable。")
            sys.exit(1)
        ...
        if result.returncode != 0:
            print(f"[ERROR] Chrome headless 失败: {result.stderr}")
            sys.exit(1)
        size_kb = os.path.getsize(args.output) / 1024
        print(f"[OK] PDF 已生成（Chrome headless）: {args.output} ({size_kb:.1f} KB)")
