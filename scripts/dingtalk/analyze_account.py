...
用法：
    python3 analyze_account.py --account 海南亿科
...
def main():
    parser = argparse.ArgumentParser(description="分析钉钉群销售情报")
    parser.add_argument("--account", required=True, help="账户名，如 千问巨量项目")
    ...
    times = [parse_time(m.get("createTime", "")) for m in messages]
...
"""
    ...
    md_lines = [
        f"# {args.account} 钉钉群销售情报 · {date_range_str}",
        "",
