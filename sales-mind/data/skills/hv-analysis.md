...
> 你需要联网获取信息。使用以下工具：
> - **search_web**：用于搜索发现信息来源，获取摘要和关键词结果
> - **fetch_webpage / jina_reader**：当已知具体URL时，用于从页面定向提取内容（优先用 jina_reader 节省 token）
> - **搜索策略**：先用 search_web 发现信息来源和线索，找到具体URL后用 fetch_webpage 或 jina_reader 深入提取
> - **多次搜索、多个关键词组合**，不要只搜一次就放弃
> - **一手来源优于二手来源**：官方博客 > 权威媒体原创报道 > 转载/聚合
> - **学术类研究对象必查arxiv**：如果研究对象涉及学术概念、算法、AI模型、技术范式等，必须通过arxiv API获取相关论文。调用方式：用 `fetch_webpage` 访问 `https://export.arxiv.org/api/query?search_query=all:关键词1+AND+all:关键词2&max_results=10`。返回XML格式，包含标题、作者、摘要、发布日期、PDF链接。可按需调整关键词组合和结果数量。找到关键论文后，用 fetch_webpage 读取论文页面（`https://arxiv.org/abs/论文ID`）获取更多细节。
> - **不要加载 web-access skill 或使用 CDP 浏览器模式**。search_web + fetch_webpage/jina_reader 足以完成绝大多数信息收集任务，使用 CDP 容易导致子Agent超时失败。
...
- **封面页**：自动生成，包含标题（28pt深蓝色）、副标题「横纵分析法深度研究报告」、作者固定为："销销"、装饰分隔线
...
六、与亿科的结合
有哪些亿科可以借鉴的或者需要规避的，有哪些是可以合作的，1500-3000字]
...
PDF文件命名为 `[研究对象名称]_横纵分析报告.pdf`，保存到用户的工作目录中。如果有更新，则需要维护好版本号，不要直接覆盖上一版
