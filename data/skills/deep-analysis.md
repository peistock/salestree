...
name: deep-analysis
description: |
  深度版横纵分析报告生成Skill。仅在用户明确要求完整版/深度版/万字报告时触发。
  要求：10000-30000字、叙事驱动、分块生成。
  触发词：完整版、深度版、万字、长报告、详细版、完整报告、详细分析。
---

# 深度分析报告生成指南

> **使用前提**：用户已在前置对话中确认需要"完整深度版"报告，或用户直接要求"深度分析""万字报告""详细版"。
> 本Skill只负责 stage 4 的报告生成，不替代 hv-analysis 的阶段1-3。
...
1. **确认PDF转换工具可用**：本 Skill 使用内置 `md_to_pdf` 工具将最终 Markdown 报告转为排版精美的 PDF。调用后立即返回，后台异步生成。。
...
搜索质量直接决定报告质量。以下规则必须遵守：

**1. 中文搜索引擎（百度/必应国内版/搜狗/360）的查询词必须用中文**
- 错误：`"THE WHOO" "后" LG生活健康 中国 业绩 财报` → 引号混中英文会导致搜索引擎拆解为乱码
- 正确：`后天气丹 LG生活健康 中国 财报` 或 `LG生活健康 中国区 美妆 业绩`

**2. 外文品牌用中文市场通用名，不用原文商标**
- "THE WHOO" → "后" 或 "后天气丹"
- "La Mer" → "海蓝之谜"
- "L'Oréal" → "欧莱雅"
- 如果中文名不确定，搜"XX品牌 中文名"

**3. 品牌级数据搜不到时，上溯母公司或行业级关键词**
- 品牌销量/市占率搜不到 → 搜母公司（如 LG生活健康）+ 中国区/美妆/护肤 + 财报/业绩
- 仍然太少 → 搜行业报告（如"中国高端护肤品市场 2025 份额"）

**4. 商业类搜索优先用中文商业媒体关键词**
- 财报、业绩、营收、市场份额、品牌战略、渠道布局、高管访谈、IPO、并购
- 避免用英文关键词在中文引擎搜索（如 revenue → 用"营收"，market share → 用"市场份额"）

**5. 学术类搜索用 arXiv/Google Scholar**
- 英文术语在学术场景下保持原文：如 "transformer architecture"、"diffusion model"
- arXiv API 搜索用英文关键词，不受中文搜索引擎限制

**6. 结果差时立即换词，不要重复同一查询**
- 首轮 3-5 个关键词组合都返回低质量结果时，立刻扩大或缩小范围
- 缩小：加限定词（时间、地域、人物）
- 扩大：去掉具体品牌名，改用行业/品类/母公司

### 搜索兜底机制（关键）

如果 search_web / multi_search 连续 2-3 轮返回的结果明显不相关（如出现 Office 365、USPS、F1 赛车、字典解释等与查询完全无关的内容），**立即停止继续搜索**，改走以下兜底路径：

**路径 1 — 直接打开搜索引擎网页手动搜**（推荐）：
- 用 `browse_open` 打开 `https://www.bing.com/search?q=查询词` 或 `https://www.baidu.com/s?wd=查询词`
- 真实浏览器看到的搜索结果通常比 SearXNG 聚合接口更准确
- 找到相关结果后，用 `fetch_webpage` 或 `jina_reader` 深入提取内容

**路径 2 — 换不同搜索引擎**：
- 中文商业信息：优先用百度（`site:baidu.com` 或直接访问 baidu.com）
- 英文/国际信息：优先用 Google（直接访问 google.com）
- 学术信息：用 arXiv / Google Scholar

**路径 3 — 换信息获取策略**：
- 品牌具体数据搜不到 → 搜母公司财报 + 行业报告
- 社交媒体营销细节搜不到 → 直接打开小红书/抖音/微博搜品牌名
- 一手信息（财报、公告）搜不到 → 直接去官网 Investor Relations 页面

**绝对不要**：在 SearXNG 返回垃圾结果时继续用 search_web 反复尝试同样的关键词。这是浪费时间。
...
报告写完后，调用 `md_to_pdf` 工具将 Markdown 转为排版精美的 PDF。

**重要：md_to_pdf 是后台异步工具**。调用后会立即返回"任务已提交后台"，Agent 不需要等待文件出现，可以直接进入最终回复。PDF 生成完成后会自动出现在 Web UI 的"产出文件"区。

### 转换流程

1. **先完成Markdown稿件**：将完整报告写为标准Markdown格式，保存为 `[研究对象]_横纵分析报告.md`
2. **调用工具**：`md_to_pdf(path="[研究对象]_横纵分析报告.md", title="研究对象名称")`
3. 工具会自动使用 WeasyPrint 生成 PDF（失败后回退到 fpdf2），并在完成后自动写入同目录 `.pdf` 文件
4. **Agent 最终回复**里告诉用户：报告已完成，Markdown 可直接查看，PDF 正在后台生成、稍后可在右侧"产出文件"区下载

### 工具内置的排版规范

`md_to_pdf` 工具已内置完整的CSS排版方案，无需手动调整：
...
报告不是随笔，每个关键事实都必须可追溯。引用规范如下：

**正文标注**：
- 每个关键事实、数据、引用他人观点后，用 `[n]` 标注引用编号
- 示例：`豆包MAU在2025年底达到3.15亿 [3]，成为国内用户规模最大的AI助手。`
- 连续引用同一来源时，每次出现都要标 `[n]`，不能只在第一次标

**信息来源清单格式**：
```
[n] 文章标题 / 作者（如有）/ 来源网站 / URL / 访问时间
```
- 示例：`[3] 豆包MAU突破3亿，字节跳动AI战略进入新阶段 / 张晓明 / 36氪 / https://36kr.com/... / 2026-01-15`
- 搜索工具返回的每条结果都有URL，必须把这些URL写进来源清单
- 如果搜不到具体文章（如App Store页面），标注为「一手来源：App Store订阅页面 / https://apps.apple.com/... / 2026-05-05」
- 严禁写笼统描述如"Kimi K2.6相关报道"，必须列出具体文章或页面

**自检**：写完报告后，随机抽查3个正文中的 `[n]`，看是否能在来源清单中找到对应URL。如果找不到，补上或删除该引用。
...
> 写作指令：我在学习「研究对象」这个概念。我希望你通过写一个寓言的方式，间接地把这个概念完整讲出来。最好一直到快结尾时，人才会慢慢意识到这个概念究竟是什么。然后在故事之后，再补一段解释，把你刚才真正要讲的概念说清楚。
>
> 要求：
> - 寓言要有场景、人物、情节，不是干巴巴的比喻
> - 故事中不要直接出现研究对象的名称，用隐喻替代
> - 快结尾时才让读者意识到「哦，原来讲的是这个」
> - 故事之后加一段 200-400 字的「解释」，点明寓言与研究对象的对应关系
> - 篇幅控制在 800-1500 字
...
报告总字数要求 10000-30000 字，单次写入会导致上下文爆炸、任务中断。**必须分段追加写入**：

1. **第一章**：用 `write_file(mode='w')` 写入寓言 + 纵向分析开头
2. **后续每一章**：用 `write_file(mode='a')` 追加写入，path 保持同一个文件名
3. **追加前不需要 read_file 回读全文**——直接写即可，避免把已写内容重新载入上下文
4. **最后一章追加完后**，调用 `md_to_pdf(path="报告.md", title="研究对象名称")` 转换
5. 调用 `md_to_pdf` 后立即结束任务，在最终回复中告诉用户 Markdown 已可查看、PDF 正在后台生成

**错误示范**：每写完一章就 `write_file` 覆盖整份报告（会把全文塞进消息历史，3-4轮后 token 耗尽）。
**正确示范**：`write_file(path='report.md', content='## 二、纵向分析\n...', mode='a')` 逐章追加。
...
- [ ] 没有触犯绝对禁区里的任何一条？
- [ ] 正文里每个关键事实都标了 `[n]` 引用编号？
- [ ] 信息来源清单列出了具体URL（不是笼统描述如"Kimi相关报道"）？
- [ ] 随机抽查3个 `[n]` 能在来源清单里找到对应URL？
- [ ] 搜不到的信息诚实标注了「暂缺」，没有编造？
- [ ] PDF排版美观、结构清晰、可读性好？
