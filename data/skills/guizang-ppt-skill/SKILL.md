---
name: ""
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮）。同时支持两种用途模式：阅读版（给人看，高密度文字 + 图文混排）与演讲版（给人讲，大字极简 + 金句页）。当用户需要制作分享 / 演讲 / 发布会 / 阅读型网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"、"阅读版 PPT"、"演讲版 PPT"时使用。
---
...
## 双模式：阅读版（给人看）vs 演讲版（给人讲）

同一份内容可以生成两种用途的 deck，版式逻辑完全不同：

| 模式 | 用途 | 信息密度 | 典型版式 | 字体策略 |
|---|---|---|---|---|
| **阅读版** | 发给别人自己读、留档、公众号/飞书转发 | 高 | 多栏文字、图文混排、列表、citation、脚注 | 正文可小至 16-18px，标题层级多 |
| **演讲版** | 主讲人现场讲、路演、发布会 | 低 | 大字 statement、hero 页、金句页、一页一个观点 | 正文最少 18px（瑞士风）/ 20px（杂志风），标题极大 |

**默认行为**：如果用户没有明确说，先问要哪种模式；如果用户说"两个都要"，基于同一份大纲生成两个 HTML：
- `项目/XXX/ppt/index-reading.html` — 阅读版
- `项目/XXX/ppt/index-presentation.html` — 演讲版

**参考图里的核心差异**（见用户提供的"01 文稿 · 给人看 / 给人讲"）：
- 阅读版把原始文稿拆成**密集图文页**：小标题 + 段落 + 要点 + 示意图，一页可承载 200-400 字。
- 演讲版把原始文稿提炼成**大字版式**：一页一个核心观点/金句，配合极简副标，单页字数控制在 30 字以内。

**不要混用**：同一页不能既想给人看又想给人讲——阅读页塞满小字会毁掉演讲节奏，演讲页信息太少会让读者觉得空洞。
...
**如果用户只给了主题或一个模糊想法**,用这 8 个问题逐个对齐后再动手。不要基于猜测就开始写 slide——一旦结构定错,后期翻修代价很高:
...
| 2 | **阅读版还是演讲版?**(给人看 / 给人讲 / 两个都要) | **必须先问**,决定信息密度、版式骨架和字号策略 |
| 3 | **受众是谁?分享场景?**(行业内部 / 商业发布 / demo day / 私享会) | 决定语言风格和深度 |
| 4 | **分享时长?** | 15 分钟 ≈ 10 页,30 分钟 ≈ 20 页,45 分钟 ≈ 25-30 页 |
| 5 | **有没有原始素材?**(文档 / 数据 / 旧 PPT / 文章链接) | 有素材就基于素材,没有就帮他搭 |
| 6 | **有没有图片或截图?希望怎么处理?** | 决定图文版式、图片槽位、截图是否需要 CleanShot X 式适配或 GPT-M 2.0 重构 |
| 7 | **想要哪套主题色?** | 杂志风 5 套(`themes.md`) / 瑞士风 4 套(`themes-swiss.md`),挑一 |
| 8 | **有没有硬约束?**(必须包含 XX 数据 / 不能出现 YY) | 避免返工 |

#### 模式选择参考(问题 2)

| 如果用户说... | 推荐模式 |
|---|---|
| "发给别人看" / "留档" / "阅读版" / "详细版" / "图文版" | **阅读版** |
| "我来讲" / "演讲版" / "发布会" / "大字版" / " keynote 风" | **演讲版** |
| "两个都要" / "一鱼两吃" / "看+讲" | **同时生成阅读版 + 演讲版** |

**时长与模式的关系**:
- 阅读版页数 ≈ 内容段落数 / 2（每页 1-2 个段落 + 配图）。
- 演讲版页数 ≈ 核心观点数（每页 1 个观点）。同一份素材，演讲版页数通常只有阅读版的 1/3 ~ 1/2。
...
**根据 Step 1 选定的风格和模式，拷贝对应的模板**到目标位置，同时在同级建一个 `images/` 文件夹准备接图片。
...
如果用户要求**同时生成阅读版 + 演讲版**，复制两份并命名区分：

```bash
# 风格 A 示例
cp "<SKILL_ROOT>/assets/template.html" "项目/XXX/ppt/index-reading.html"
cp "<SKILL_ROOT>/assets/template.html" "项目/XXX/ppt/index-presentation.html"

# 风格 B 示例
cp "<SKILL_ROOT>/assets/template-swiss.html" "项目/XXX/ppt/index-reading.html"
cp "<SKILL_ROOT>/assets/template-swiss.html" "项目/XXX/ppt/index-presentation.html"
```
...
**注意**:阅读版与演讲版**不能用同一份 HTML 混排**。阅读版塞满小字，演讲版一页一金句，必须分文件。
...
**阅读版（给人看）**：
- 优先使用信息承载力强的版式：多栏文字、图文混排、列表、时间线、对比表、矩阵。
- 风格 A 可多使用 Layout 4（左文右图）、Layout 6（流水线）、Layout 9（并列对比）、Layout 10（图文混排）。
- 风格 B 可多使用 S02/S11（时间线）、S04/S15/S16（矩阵/多卡）、S07/S20（数据表）、S08（对比）、S21（规格表）。
- 一页可放 2-4 个要点，每个要点配 1-2 句说明；必要时加脚注或 citation。

**演讲版（给人讲）**：
- 优先使用视觉冲击力强的版式：封面、幕封、大引用、大字 statement、数据 hero、金句页。
- 风格 A 可多使用 Layout 1（封面）、Layout 2（幕封）、Layout 3（数据大字报）、Layout 7（悬念收束）、Layout 8（大引用）。
- 风格 B 可多使用 S01/S09/S10（statement）、S03（分屏论点）、S06/S18（数据 hero）、S12（宣言页）、S22（大图 hero）。
- 一页只讲一个核心观点，正文不超过 3 行，每行不超过 15 字。

**同时生成双版本时**：先按阅读版把信息拆全，再按演讲版把核心观点提炼成金句；不要先写演讲版再反向扩写成阅读版，会损失信息结构。
